...
### 1.2.2 (2026-01-01) - 当前版本

**🔧 改进**:
- 诊断系统优化：跳过 `sections`（原版节名）的未使用检测，因为游戏默认已注册这些节名
- 诊断精准性进一步提升：同时排除 `registerType` 和原版 `sections` 中的节名，避免不必要的提示
...
**✨ 新增功能**:
- 引用跳转与预览开关：`enableJumpToDefinition` 配置，支持直接跳转/预览两种导航方式
- 面包屑导航功能：在 VS Code 顶部面包屑显示所有 INI 节名，快速定位；支持显示节的同行注释信息
- 补充 18 个词典词条：国家配置、特殊配置节名等注册表节

**🔧 改进**:
- 统计功能增强：新增重复节检测，显示重复节名和行号位置
- 诊断精准性改进：修复布尔值(`yes`/`no`)和浮点值误报问题，限制值引用检查只在注册表键中进行
- 悬浮提示优化：新增 `showEmptyReferenceHint` 配置，控制是否显示"未找到引用"文案，使悬浮更简洁
...
详见 [CHANGELOG_v1.2.0.md](docs/versions/CHANGELOG_v1.2.0.md)
